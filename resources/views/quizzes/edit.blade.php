...
@section('content')
        ...
        <div>
            <div class="card h-full shadow">
                <div class="border-b border-gray-300 px-5 py-4 flex items-center w-full justify-between">
                    <div>
                        <h4 class="font-semibold text-base">Edit Quiz</h4>
                    </div>
                </div>
                <div class="container mx-auto py-8 px-5">

                    <form action="{{ route('quizzes.update', $quiz->id) }}" method="POST" id="quiz-form">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            ...
                            <select id="module_id" name="module_id"
                                ...
                                @foreach ($modules as $module)
                                    <option value="{{ $module->id }}"
                                        {{ $quiz->course_id == $module->id ? 'selected' : '' }}>
                                        {{ $module->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="accordionExample" class="grid grid-cols-3 gap-3">
                                @foreach (json_decode($quiz->questions, true) as $index => $question)
                                            ...
                                            <div class="accordion-body px-4 py-3 border-t border-gray-400">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    @foreach ($question['answers'] as $answerIndex => $answer)
                                                        <input type="text" name="answers_{{ $index }}[]"
                                                            class="block w-full mt-1 p-2 border border-gray-300 rounded"
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="number" name="correct_{{ $index }}"
                                                        class="block w-full mt-1 p-2 border border-gray-300 rounded"
                                                        value="{{ $question['correct'] }}" required min="1"
                                                        max="4" placeholder="Correct Answer (1-4)">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="block text-gray-700">Image (optional)</label>
                                                    <input type="text" name="image_{{ $index }}"
                                                        class="block w-full mt-1 p-2 border border-gray-300 rounded"
                                                        value="{{ $question['image'] ?? '' }}" placeholder="Image URL">
                                                </div>
                                                <button type="button"
                                                    class="remove-question px-4 py-2 bg-red-600 text-white rounded">Hapus
                                                    Pertanyaan</button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <button type="button" id="add-question"
                                class="px-4 py-2 bg-blue-600 text-white rounded mt-4">Tambah
                                Pertanyaan</button>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            let questionIndex = {{ count(json_decode($quiz->questions, true)) }};
