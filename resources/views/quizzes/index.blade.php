...
@section('content')
        ...
        <div class="xl:col-span-2">
            <div class="card h-full shadow">
                <div class="border-b border-gray-300 px-5 py-4 flex items-center w-full justify-between">
                    <!-- title -->
                    <div>
                        <h4 class="font-semibold text-base">Quiz Table</h4>
                    </div>
                    <div>
                        <!-- button -->
                        <div class="leading-4">
                        ...
                        <table class="text-left w-full whitespace-nowrap">
                            <thead class="text-gray-700 sticky top-0">
                                <tr>
                                    ...
                                    <th scope="col" class="border-b bg-gray-100 px-6 py-3 cursor-default">Questions</th>
                                    <th scope="col" class="border-b bg-gray-100 px-6 py-3 cursor-default">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($quizzes as $quiz)
                                    <tr class="hover:bg-slate-100 transition-all ease-in-out">
                                        <td class="border-b border-gray-300 font-medium py-3 px-6 text-left">
                                            {{ $quiz->module->name }}</td>
                                        <td class="border-b border-gray-300 font-medium py-3 px-6 text-left">
                                            {{ count(json_decode($quiz->questions, true)) }} questions</td>
                                        <td class="border-b border-gray-300 font-medium py-3 px-6 text-left">
                                            <a href="{{ route('quizzes.edit', $quiz->id) }}" class="btn group btn-sm">
                                            ...
                                            <button class="btn group btn-sm" onclick="deleteQuiz({{ $quiz->id }})">
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                            ...
                                            <form id="delete-form-{{ $quiz->id }}"
                                                action="{{ route('quizzes.destroy', $quiz->id) }}" method="POST"
                                                class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
    ...
    <script>
        function deleteQuiz(id) {
            Swal.fire({
                title: 'Apakah Anda yakin ingin menghapus kuis ini?',
