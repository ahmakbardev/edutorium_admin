...
@section('content')
        ...
        <div class="">
            <div class="card h-full shadow">
                <div class="border-b border-gray-300 px-5 py-4 flex items-center w-full justify-between">
                    <div>
                        <h4 class="font-semibold text-base">Penilaian Tugas Akhir untuk {{ $user->name }}</h4>
                    </div>
                </div>

                <div class="container mx-auto py-8 px-5">
                    <form action="{{ route('tugas_akhir_assessments.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ $user->id }}">
                        <input type="hidden" name="tugas_akhir_id" value="{{ $submission->tugas_akhir_id }}">
                        <div class="mb-4">
    ...
    <div class="offcanvas translate-x-full fixed top-0 right-0 border-l border-gray-300 w-full max-w-xl transition-all duration-300 transform h-full invisible bg-white z-50"
